...
function makeAgent(i,n,traitBias={}){
  ...
  return{id:uid(),name:NAMES[i%NAMES.length]+"-"+String(Math.floor(Math.random()*900)+100),
...
async function agentTick(agent,actions,ctx){
  const td=B5K.map(k=>`${B5[k].full}:${agent.traits[k]}/10`).join(", ");
  const mems=agent.memories.slice(-5).map(m=>`- ${m}`).join("\n");
  ...
  try{
    const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:350,
        system:`You are ${agent.name}, a ${agent.role} (${agent.culture}). Big Five: ${td}.\n${mems?"Memories:\n"+mems:""}\n${recent?"Others:\n"+recent:""}\n${ctx}\nRespond ONLY with JSON: {"thought":"1-2 sentences","action":"1 sentence","sentiment":<-1 to 1>,"stance":"agree|disagree|neutral|cooperate|defect"}`,
    ...
    const a=agent.traits.A/10,e=agent.traits.E/10,n=agent.traits.N/10;
    return{thought:`I'm considering this ${n>0.6?"anxiously":"carefully"}.`,action:e>0.5?`I ${a>0.5?"support":"question"} this approach.`:"I'll observe.",sentiment:cl((a-0.5)*0.6+(0.5-n)*0.4+(Math.random()-0.5)*0.3,-1,1),stance:a>0.5?"cooperate":"neutral"};
  }
}
...
function Fingerprint({traits,size=44}){
  ...
  useEffect(()=>{const c=ref.current;if(!c)return;const ctx=c.getContext("2d"),s=size,cx=s/2,cy=s/2,r=s*0.42;ctx.clearRect(0,0,s,s);
    ...
    <div style={{display:"flex",alignItems:"center",gap:0,marginBottom:28}}>
      {steps.map((s,i)=>(
        <div key={i} style={{display:"flex",alignItems:"center",flex:i<steps.length-1?1:"none"}}>
          <div style={{width:30,height:30,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,fontFamily:FM,
...
export default function HumanityPhase4(){
  ...
  const runCondition=useCallback(async(condIdx)=>{
    ...
    setConditionResults(prev=>{
      const updated=[...prev];
      updated[condIdx]={conditionName:cond.name,agents:finalAgents,avgSentiment:avgS,polarization:cl(Math.sqrt(varS)*2,0,1),
          ...
          {step===3&&(
            <div>
              ...
              <Btn variant="secondary" onClick={handleGenerateHypotheses} disabled={loadingHypo}
                style={{width:"100%",marginBottom:16,padding:"14px 0",
                    ...
                    <div key={h.id||i} style={{padding:14,background:T.panel,border:`1px solid ${T.borderHi}`,borderRadius:10,animation:`fadeUp .3s ease ${i*0.08}s both`}}>
                      ...
                      <div style={{marginLeft:30,display:"flex",flexDirection:"column",gap:3}}>
                        ...
                        <div style={{display:"flex",gap:8}}>
                          ...
                          <span style={{padding:"1px 6px",borderRadius:5,fontSize:9,fontFamily:FM,
          ...
          <div style={{padding:"10px 20px",borderBottom:`1px solid ${T.border}`,background:T.panelTop,display:"flex",alignItems:"center",gap:16,flexShrink:0}}>
            <div style={{flex:1}}>
              ...
              <div style={{height:5,background:T.border,borderRadius:3,overflow:"hidden",animation:running?"progressPulse 2s ease infinite":"none"}}>
                <div style={{height:"100%",width:`${simDone?100:(((simCondition*config.ticks+tick)/(config.conditions.length*config.ticks))*100)}%`,
          ...
          <div style={{flex:1,display:"grid",gridTemplateColumns:"1fr 340px",overflow:"hidden"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",overflow:"hidden"}}>
              ...
              <div style={{display:"flex",gap:4,padding:"8px 14px",borderBottom:`1px solid ${T.border}`,flexShrink:0}}>
                {config.conditions.map((c,i)=>(
                  <div key={c.id} style={{padding:"4px 12px",borderRadius:6,fontSize:10,fontFamily:FM,fontWeight:600,
              ...
              <div ref={graphRef} style={{flex:1,position:"relative",margin:"6px 10px",borderRadius:10,overflow:"hidden",background:T.bgAlt,border:`1px solid ${T.border}`}}>
                <svg width={graphSize.w} height={graphSize.h}>
                  ...
                  {groups.map(g=>{const ms=agents.filter(a=>g.members.includes(a.id));if(ms.length<2)return null;const cx=ms.reduce((s,m)=>s+m.x,0)/ms.length*graphSize.w;const cy=ms.reduce((s,m)=>s+m.y,0)/ms.length*graphSize.h;const mr=Math.max(25,...ms.map(m=>Math.hypot(m.x*graphSize.w-cx,m.y*graphSize.h-cy)))+18;
                    return <circle key={g.id} cx={cx} cy={cy} r={mr} fill={g.color+"06"} stroke={g.color+"22"} strokeWidth={1} strokeDasharray="4 4"/>;
                  })}
                  {agents.map(a=>{const px=a.x*graphSize.w,py=a.y*graphSize.h,col=sCol(a.sentiment);return(
              ...
              {config.hypotheses.map((h,i)=>(
                <div key={i} style={{padding:12,background:T.panel,border:`1px solid ${T.border}`,borderRadius:10,marginBottom:6,display:"flex",gap:10}}>
                  ...
                  <div>
                    ...
                    <span style={{display:"inline-block",marginTop:3,padding:"1px 6px",borderRadius:5,fontSize:9,fontFamily:FM,
                      background:h.direction==="increase"?T.greenS:h.direction==="decrease"?T.roseS:T.amberS,
                      color:h.direction==="increase"?T.green:h.direction==="decrease"?T.rose:T.amber,
                    }}>{h.direction==="increase"?"↑ INCREASE":h.direction==="decrease"?"↓ DECREASE":"— NO CHANGE"}</span>
                  </div>
